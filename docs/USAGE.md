...
Hugging Face Space 额外提供全局音高滑块，范围为 `-3` 到 `+3` 半音，默认 `+0.5` 半音。它在生成后使用高质量重采样，用于微调“偏低沉/偏尖”的整体听感；升高音高时语速会略快（`+0.5` 半音约快 3%），但不会产生保时长相位移调常见的混响/合唱感，也不会改变普通话一至四声的语言学声调。历史训练记录中，v1 曾出现音调偏高，而特殊音效类训练素材也可能拉偏音高，因此这里只采用较小的默认提升。
