...
#### Local `.venv` workflow for this repository

This repository is also prepared for a local Windows workflow that avoids Hugging Face, uses the Aliyun PyPI mirror, downloads model assets from ModelScope, and keeps local training data out of git.

```pwsh
pwsh -File .\scripts\init-venv.ps1
pwsh -File .\scripts\prepare-smbw-training.ps1
pwsh -File .\scripts\start-webui.ps1
```

The SMBW preparation script only uses `CNzh` and `TWzh` from the archive and writes the extracted dataset under `.\data\smbw_zh_train`.
...
copilot --resume=7d98846d-8de4-4336-8538-5e81531eeda1